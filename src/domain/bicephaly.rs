...
impl<T> Node<T> {
    conditional_const!(
        ...
        fn new(value: T) -> Self {
            Self {
                value,
                next_available: AtomicPtr::new(std::ptr::null_mut()),
                next_in_use: AtomicPtr::new(std::ptr::null_mut()),
            }
        }
    );
}
...
impl<T> Bicephaly<T> {
    conditional_const!(
        ...
        fn new() -> Self {
            Self {
                available_head: AtomicPtr::new(std::ptr::null_mut()),
                in_use_head: AtomicPtr::new(std::ptr::null_mut()),
